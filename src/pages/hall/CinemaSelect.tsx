...
const CinemaSelect = ({ selectedId, onSelect }: CinemaSelectProps) => {
    ...
    if (isLoading) return (
        <div className="h-10 px-4 flex items-center bg-white/5 rounded-xl border border-white/10 animate-pulse">
            <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Loading cinemas...</span>
        </div>
    );
    ...
    if (isError || !cinemas || cinemas.length === 0) return (
        <div className="h-10 px-4 flex items-center bg-red-500/10 rounded-xl border border-red-500/20">
            <span className="text-[10px] font-black uppercase text-red-400 tracking-widest italic">No cinemas found!</span>
        </div>
    );
    ...
    return (
        <div className="flex flex-col gap-1.5">
            ...
            <select
                ...
                style={{ backgroundImage: 'url("data:image/svg+xml;charset=UTF-8,%3csvg xmlns=\'https://www.w3.org\' viewBox=\'0 0 24 24\' fill=\'none\' stroke=\'white\' stroke-width=\'2\' stroke-linecap=\'round\' stroke-linejoin=\'round\'%3e%3cpolyline points=\'6 9 12 15 18 9\'%3e%3c/polyline%3e%3c/svg%3e")', backgroundRepeat: 'no-repeat', backgroundPosition: 'right 1rem center', backgroundSize: '1em' }}
            >
                <option value="" disabled className="bg-slate-950 text-slate-500">Choose location...</option>
                {cinemas.map((cinema) => (

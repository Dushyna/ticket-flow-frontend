...
const ShowtimeTimeline = ({hallId, onEdit}: ShowtimeTimelineProps) => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            dispatch(showNotification({ message: t('timeline.success_delete'), type: "success" }));
            setShowtimeToDelete(null);
        } catch (err) {
            const fetchError = err as FetchBaseQueryError;
            const errorMessage = (fetchError.data as { message?: string })?.message
                || t('timeline.error_delete');
    ...
    return (
        <div className="mt-16 w-full animate-in fade-in slide-in-from-bottom-6 duration-700">
            <div className="flex items-center justify-between mb-8 border-l-4 border-indigo-500 pl-6">
                <div>
                    <h2 className="text-2xl font-black uppercase italic tracking-tighter text-white">
                        {t('timeline.title_hall')} <span className="text-indigo-500">{t('timeline.title_timeline')}</span>
                    </h2>
                ...
                <div className="bg-indigo-500/10 border border-indigo-500/20 px-4 py-2 rounded-2xl">
                    <span className="text-indigo-400 font-black text-sm uppercase">
                        {existingShowtimes?.length || 0} {t('timeline.status_planned')}
                    </span>
                </div>
            </div>
            ...
            <div className="grid gap-4">
                {isShowtimesLoading ? (
                    ...
                    <div className="bg-white/5 border border-dashed border-white/10 rounded-[40px] py-16 text-center">
                        <p className="text-slate-600 font-black uppercase text-xs tracking-[0.3em]">{t('timeline.empty_msg')}</p>
                    </div>
                ) : (
                    sortedShowtimes?.map((st: Showtime) => (
                        <div key={st.id} className="group bg-white/5 hover:bg-white/[0.07] border border-white/10 p-6 rounded-[32px] flex items-center justify-between transition-all backdrop-blur-md">
                            <div className="flex items-center gap-6">
                                <div className="w-14 h-14 bg-black/40 rounded-2xl flex items-center justify-center text-indigo-400 group-hover:scale-110 transition-transform duration-300">
                                    <Film size={24} />
                                </div>
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 mt-1">
                                        <div className="flex items-center gap-1.5 text-indigo-400 font-bold text-xs bg-indigo-500/10 px-2 py-0.5 rounded-lg">
                                            <Clock size={12} />
                                            {new Date(st.startTime).toLocaleTimeString(i18n.language, { hour: '2-digit', minute: '2-digit' })}
                                            —
                                            {new Date(st.endTime).toLocaleTimeString(i18n.language, { hour: '2-digit', minute: '2-digit' })}
                                        </div>
                                        <span className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">
                                            {new Date(st.startTime).toLocaleDateString(i18n.language)}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-end mr-6">
                                <div className="flex items-baseline gap-1 text-white">
                                    <span className="text-slate-500 font-bold text-xs">{t('common.currency')}</span>
                                    <span className="text-2xl font-black">{st.basePrice}</span>
                                </div>
                                <p className="text-[8px] text-slate-600 font-black uppercase tracking-[0.2em]">{t('timeline.price_label')}</p>
                            </div>

                            <div className="flex items-center gap-4">
                                <button
                                    onClick={() => onEdit(st)}
                                    className="p-3 bg-indigo-500/10 text-indigo-400 rounded-xl hover:bg-indigo-500 hover:text-white transition-all md:opacity-0 group-hover:opacity-100"
                                >
                                    <Edit2 size={18} />
                                </button>
                                <button
                                    onClick={() => setShowtimeToDelete(st)}
                                    className="p-4 bg-red-500/10 text-red-500 rounded-2xl hover:bg-red-500 hover:text-white transition-all md:opacity-0 group-hover:opacity-100"
                                >
                                    <Trash2 size={20} />
                                </button>
                            </div>
            ...
            {showtimeToDelete && (
                <ConfirmModal
                    title={t('timeline.modal_delete_title')}
                    message={t('timeline.modal_delete_msg', { title: showtimeToDelete.movieTitle })}
